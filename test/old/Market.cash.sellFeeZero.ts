...
describe("Market", function () {
  describe("cash.sellFeeZero", function () {
    it("cash revert", async function () {
      const info = (await loadFixture(deployAllContracts)).sellFeeZeroappOperatorAllContractInfo;
      ...
      // cash not tokenid
      await expect(info.market.connect(user1).cash(5, 100)).revertedWithCustomError(info.mortgageNFT, "ERC721NonexistentToken");
      // cash deleted tokenid
      await expect(info.market.connect(user1).cash(1, 10)).revertedWithCustomError(info.mortgageNFT, "ERC721NonexistentToken");
      ...
      let result = await info.market.connect(user2).cash.staticCall(2, getTokenAmountWei(10))
      expect(result).gt(0)
    });
    ...
    it("cash", async function () {
      ...
      let result = await info.market.connect(user1).cash.staticCall(1, getTokenAmountWei(1000))
      expect(result).eq(0)
      ...
      expect(nftOwnerT1_2_add).eq(0)
      expect(nftOwnerT1_1_add).eq(0)

      expect(market_payToken_0 - market_payToken_1)
        .eq(user1_payToken_1 - user1_payToken_0)
        .eq(curve_sell_0 - curve_redeem_0)
        .eq(userGetPayToken_0);
      ...
      expect(nftOwnerT1_2_add_1).eq(0)
      expect(nftOwnerT1_1_add_1).eq(0)

      expect(user1_payToken_2 - user1_payToken_2_before).gt(0);

      expect(market_payToken_1 - market_payToken_2)
        .eq(user1_payToken_2 - user1_payToken_2_before)
