...
describe("Market", function () {
  describe("buyAndSell.sellFeeZero", function () {
    let data = [
      {
    ...
    it("buy one and sell one", async function () {
      for (let i = 0; i < data.length; i++) {
        ...
        expect(nftOwner1AddPayToken2).eq(0)
        expect(nftOwner2AddPayToken2).eq(0)
        expect(marketPayTokenSell1).eq(0);
        expect(marketPayToken2).eq(user1PayTokenSellAdd);
      }
    });
  });
});
