...
describe("Market", function () {
    describe("multiplyAdd", function () {
        it("multiplyAdd revert", async function () {
            ...
            // no tokenid
            await expect(
                info.market.multiplyAdd(1, getTokenAmountWei(1000)),
            ).revertedWithCustomError(info.mortgageNFT, "ERC721NonexistentToken");

            let result = await info.market.connect(user1).multiplyNew.staticCall(tid_1, getTokenAmountWei(1000), { value: BigInt(10) ** BigInt(20) });
            await info.market.connect(user1).multiplyNew(tid_1, getTokenAmountWei(1000), { value: result.payTokenAmount });

            // not owner
            await expect(
                info.market.connect(user2).multiplyAdd(result.nftTokenId, BigInt(10) ** BigInt(20))
            ).revertedWith("AOE");

            // value < need
            await expect(
                info.market.connect(user1).multiplyAdd(result.nftTokenId, BigInt(10) ** BigInt(20), { value: BigInt(10) ** BigInt(10) })
            ).revertedWith("VE");

            // multiplyAdd 0
            await expect(info.market.connect(user1).multiplyAdd(result.nftTokenId, 0)).revertedWith("TAE");

            // multiplyAdd 100w
            await expect(info.market.connect(user1).multiplyAdd(result.nftTokenId, BigInt(10) ** BigInt(26))).revertedWithPanic(0x11);
        });

        it("multiplyAdd revert erc20", async function () {
            ...
            // no tokenid
            await expect(
                app2_market.multiplyAdd(1, getTokenAmountWei(1000)),
            ...
            let resultNew = await app2_market.connect(user1).multiplyNew.staticCall(tid_1, BigInt(10) ** BigInt(20))
            await app2_payToken.connect(user1).approve(app2_market.getAddress(), resultNew.payTokenAmount)
            await app2_market.connect(user1).multiplyNew(tid_1, BigInt(10) ** BigInt(20));

            // not owner
            await expect(
                app2_market.connect(user2).multiplyAdd(resultNew.nftTokenId, BigInt(10) ** BigInt(20))
            ).revertedWith("AOE");

            // value < need
            await app2_payToken.transfer(user1.address, getTokenAmountWei(1000))
            await app2_payToken.connect(user1).approve(app2_market.getAddress(), MaxUint256)
            let result = await app2_market.connect(user1).multiplyAdd.staticCall(resultNew.nftTokenId, BigInt(10) ** BigInt(20))
            await app2_payToken.connect(user1).approve(app2_market.getAddress(), result - BigInt(1))
            await expect(
                app2_market.connect(user1).multiplyAdd(resultNew.nftTokenId, BigInt(10) ** BigInt(20))
            ).revertedWithCustomError(app2_payToken, "ERC20InsufficientAllowance");

            // multiplyAdd 0
            await expect(app2_market.connect(user1).multiplyAdd(resultNew.nftTokenId, 0)).revertedWith("TAE");

            // multiplyAdd 100w
            await expect(app2_market.connect(user1).multiplyAdd(resultNew.nftTokenId, BigInt(10) ** BigInt(26))).revertedWithPanic(0x11);
        });

        it("multiplyAdd with eth", async function () {
            const info = (await loadFixture(deployAllContracts));
            ...
            let multiplyAddAmount = BigInt(10) ** BigInt(18) * BigInt(10000)
            let result2 = await app2_market.connect(user1).multiplyAdd.staticCall(result.nftTokenId, multiplyAddAmount, { value: multiplyAddAmount })
            let res2 = await app2_market.connect(user1).multiplyAdd(result.nftTokenId, multiplyAddAmount, { value: result2 })
            ...
            expect(result2).eq("20614306328592040")
            ...
            let curve_amount_buy_2 = await app2_market.getPayTokenAmount(multiplyNewAmount, multiplyAddAmount)
            let curve_amount_multiply_2 = await app2_market.getPayTokenAmount(multiplyNewAmount, multiplyAddAmount)

            expect(curve_amount_buy_2).eq(curve_amount_multiply_2)
            expect(result2).eq(nft_fee_2 + app_owner_fee_2 + platform_mortgage_fee_2)

            expect(curve_amount_multiply_2 / (nft_fee_2 + app_owner_fee_2 + platform_mortgage_fee_2)).eq(50)

            expect(curve_amount_buy_2 / nft_fee_2).eq(100000 / app2_fees.nftOwnerBuyFee)
            expect(curve_amount_buy_2 / platform_mortgage_fee_2).eq(100000 / platformMortgageFee)
            expect(nft_fee_2 / platform_mortgage_fee_2).eq(4)
            expect(app_owner_fee_2 / platform_mortgage_fee_2).eq(5)

            expect((nft_2_owner_eth_balance_4 - nft_2_owner_eth_balance_3) / (nft_1_owner_eth_balance_4 - nft_1_owner_eth_balance_3)).eq(9)

            expect(app2_market_eth_balance_4).eq(app2_market_eth_balance_3).eq(0)

            expect(await app2_market.totalSupply(tid)).eq(multiplyNewAmount + multiplyAddAmount)
            expect(user1_token_balance_4).eq(user1_token_balance_3).eq(0)
            expect(app2_market_token_balance_4).eq(app2_market_token_balance_3 + multiplyAddAmount)

            expect(await app2_mortgageNFT.totalSupply()).eq(1)
            expect(await app2_mortgageNFT.ownerOf(result.nftTokenId)).eq(user1.address)
            expect((await app2_mortgageNFT.info(result.nftTokenId)).amount).eq(multiplyNewAmount + multiplyAddAmount)

        });

        it("multiplyAdd with eth | value > need", async function () {
            const info = (await loadFixture(deployAllContracts));
            ...
            let multiplyAddAmount = BigInt(10) ** BigInt(18) * BigInt(10000)
            let result2 = await app2_market.connect(user1).multiplyAdd.staticCall(result.nftTokenId, multiplyAddAmount, { value: multiplyAddAmount })
            let res2 = await app2_market.connect(user1).multiplyAdd(result.nftTokenId, multiplyAddAmount, { value: result2 * BigInt(2) })
            let ress2 = await res2.wait()
            let gas2 = ress2!.gasPrice * ress2!.gasUsed
        ...
        it("multiplyAdd with eth | value > need | no nft buy fee", async function () {
            const info = (await loadFixture(deployAllContracts));
            ...
            expect(result2).eq("12368583797155224")
            expect(user1_eth_balance_4).eq(user1_eth_balance_3 - result2 - gas2)
            ...
            expect(curve_amount_buy_2).eq(curve_amount_multiply_2)
            expect(result2).eq(app_owner_fee_2 + platform_mortgage_fee_2)

            expect(curve_amount_multiply_2 / (app_owner_fee_2 + platform_mortgage_fee_2)).eq(83)
        ...
        it("multiplyAdd with eth | value > need | no nft buy fee | no nft", async function () {
            const info = (await loadFixture(deployAllContracts));
        ...
        it("multiplyAdd with eth | value > need | no app buy fee", async function () {
            const info = (await loadFixture(deployAllContracts));
            ...
            expect(result2).eq("14430014430014428")
            expect(user1_eth_balance_4).eq(user1_eth_balance_3 - result2 - gas2)
            ...
            expect(curve_amount_multiply_2 / (nft_fee_2 + app_owner_fee_2 + platform_mortgage_fee_2)).eq(71)
        ...
        it("multiplyAdd with eth | value > need | no app mortgage fee", async function () {
            const info = (await loadFixture(deployAllContracts));
            ...
            expect(result2).eq("16491445062873632")
            expect(user1_eth_balance_4).eq(user1_eth_balance_3 - result2 - gas2)
            ...
            expect(curve_amount_multiply_2 / (nft_fee_2 + app_owner_fee_2 + platform_mortgage_fee_2)).eq(62)

            expect(curve_amount_buy_2 / nft_fee_2).eq(100000 / app2_fees.nftOwnerBuyFee)
            expect(curve_amount_buy_2 / platform_mortgage_fee_2).eq(100000 / platformMortgageFee)
            expect(nft_fee_2 / platform_mortgage_fee_2).eq(4)
            expect(app_owner_fee_2 / platform_mortgage_fee_2).eq(3)
        ...
        it("multiplyAdd with eth | value > need | no platform mortgage fee", async function () {
            const info = (await loadFixture(deployAllContracts));
            ...
            expect(result2).eq("18552875695732836")
            expect(user1_eth_balance_4).eq(user1_eth_balance_3 - result2 - gas2)
            ...
            expect(curve_amount_buy_2).eq(curve_amount_multiply_2)
            expect(result2).eq(nft_fee_2 + app_owner_fee_2)

            expect(curve_amount_multiply_2 / (nft_fee_2 + app_owner_fee_2)).eq(55)

            expect(curve_amount_buy_2 / nft_fee_2).eq(100000 / app2_fees.nftOwnerBuyFee)
            expect(curve_amount_buy_2 / app_owner_fee_2).eq(100000 / (app2_fees.appOwnerBuyFee + app2_fees.appOwnerMortgageFee))
        ...
        it("multiplyAdd with erc20", async function () {
            const info = (await loadFixture(deployAllContracts));
            ...
            await app2_payToken.connect(user1).approve(await app2_market.getAddress(), result.payTokenAmount)
            ...
            // multiplyAdd
            let multiplyAddAmount = BigInt(10) ** BigInt(18) * BigInt(10000)
            await app2_payToken.transfer(await user1.getAddress(), multiplyAddAmount)
            await app2_payToken.connect(user1).approve(await app2_market.getAddress(), MaxUint256)
            let result2 = await app2_market.connect(user1).multiplyAdd.staticCall(result.nftTokenId, multiplyAddAmount)
            await app2_payToken.connect(user1).approve(await app2_market.getAddress(), result2)
            ...
            let res2 = await app2_market.connect(user1).multiplyAdd(result.nftTokenId, multiplyAddAmount)
            let ress2 = await res2.wait()
            let gas2 = ress2!.gasPrice * ress2!.gasUsed
            ...
            expect((nft_2_owner_pay_token_balance_4 - nft_2_owner_pay_token_balance_3) / (nft_1_owner_pay_token_balance_4 - nft_1_owner_pay_token_balance_3)).eq(9)

            expect(app2_market_pay_token_balance_4).eq(app2_market_pay_token_balance_3).eq(0)
        ...
        it("multiplyAdd with erc20 | approve > need", async function () {
            const info = (await loadFixture(deployAllContracts));
            ...
            await app2_payToken.connect(user1).approve(await app2_market.getAddress(), MaxUint256)
            let result = await app2_market.connect(user1).multiplyNew.staticCall(tid, multiplyNewAmount)
            await app2_payToken.connect(user1).approve(await app2_market.getAddress(), result.payTokenAmount * BigInt(2))
            ...
            await app2_payToken.connect(user1).approve(await app2_market.getAddress(), MaxUint256)
            let result2 = await app2_market.connect(user1).multiplyAdd.staticCall(result.nftTokenId, multiplyAddAmount)
            await app2_payToken.connect(user1).approve(await app2_market.getAddress(), result2 * BigInt(2))
        ...
        it("multiplyAdd with erc20 | approve > need | no nft buy fee", async function () {
            const info = (await loadFixture(deployAllContracts));
        ...
        it("multiplyAdd with erc20 | approve > need | no nft buy fee | no nft", async function () {
            const info = (await loadFixture(deployAllContracts));
        ...
        it("multiplyAdd with erc20 | approve > need | no app buy fee", async function () {
            const info = (await loadFixture(deployAllContracts));
        ...
        it("multiplyAdd with erc20 | approve > need | no app mortgage fee", async function () {
            const info = (await loadFixture(deployAllContracts));
        ...
        it("multiplyAdd with erc20 | approve > need | no platform mortgage fee", async function () {
            const info = (await loadFixture(deployAllContracts));
